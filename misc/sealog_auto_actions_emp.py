#!/usr/bin/env python3
'''
FILE:           sealog_auto_actions_emp.py
...
DESCRIPTION:    This service listens for new events submitted to Sealog
                ...
                This service listens for "In water" and "On deck"
                milestones and if a lowering is currently active it will set
                the start/stop time to the time of the event.

                This service listens for "Out of water" and "Aborted"
                milestones and if a lowering is currently active it will
                set the corresponding lowering milestone time to the time
                of the event.
...
from misc.python_sealog.settings import (
    API_SERVER_URL, WS_SERVER_URL, HEADERS, EVENTS_API_PATH, LOWERINGS_API_PATH,
)
...
WATCH_CHANGE_START_MILESTONE = 'In water'


def _handle_vehicle_event(event):
    '''
    The function handle auto actions for the VEHICLE event_value.  It uses the
    included event_options to set the lowering start/stop times and dive
    milestones.
    '''

    milestone = None

    # if event has milestone event_option, pass milestone to _set_milestones
    for option in event.get('event_options', []):
        if option.get('event_option_name') == "milestone":
            milestone = option.get('event_option_value')
            break
...
def _set_milestones(event, evt_milestone):  # pylint: disable=too-many-branches,too-many-statements
    ...
    if milestones_to_set:
        ...
        for milestone in milestones_to_set:
            ...
            milestone_reset = (
                milestone.get('reset', False) if isinstance(milestone, dict) else False
            )

            if (
                'milestones' not in lowering['lowering_additional_meta'] or
    ...
    try:
        requests.patch(
            API_SERVER_URL + LOWERINGS_API_PATH + '/' + lowering['id'],
            headers=HEADERS,
...
def _update_first_watch_change(event):
    '''
    Update the timestamp of the first WATCH CHANGE event to match the 'In water' milestone.
    '''
    ...
    if watch_change_events:
        requests.patch(
            API_SERVER_URL + EVENTS_API_PATH + '/' + watch_change_events[-1]['id'],
            headers=HEADERS,
