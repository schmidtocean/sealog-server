...
class FrontCoverTemplate(PageTemplate):  # pylint: disable=too-few-public-methods
    '''
    ...
    def afterDrawPage(self, canv, _doc):
        '''
        ...
        #     canv.drawCentredString(self.page_width / 2, self.page_height - 3.35 * cm, self.subtitle)  # noqa: E501
        #     canv.setFont('Helvetica', 10)
        #     canv.drawCentredString(self.page_width / 2, self.page_height - 3.85 * cm, 'Generated: ' + datetime.utcnow().replace(microsecond=0).strftime("%c"))  # noqa: E501
        #     canv.drawImage(os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)),'../assets/images/Subastian_Profile.png')), self.page_width / 2 - 140 / 2, self.page_height - 7 * cm, width=140, height= 140 * 233/400, mask='auto')  # noqa: E501

        # else:
        #     canv.setFont('Helvetica', 10)
        #     canv.drawCentredString(self.page_width / 2, self.page_height - 3.25 * cm, 'Generated: ' + datetime.utcnow().replace(microsecond=0).strftime("%c"))  # noqa: E501
        #     canv.drawImage(os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)),'../assets/images/Subastian_Profile.png')), self.page_width / 2 - 140 / 2, self.page_height - 6.35 * cm, width=140, height= 140 * 233/400, mask='auto')  # noqa: E501
        ...
        if self.subtitle:
            canv.drawImage(os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../assets/images/Subastian_Profile.png')), 2.5 * cm, self.page_height - 4.5 * cm, width=80 * 1153/675, height=80, mask='auto')  # noqa: E501
            canv.setFont('Helvetica-Oblique', 14)
            canv.drawString(7.5 * cm, self.page_height - 3.35 * cm, self.subtitle)
            canv.setFont('Helvetica', 10)
            canv.drawString(7.5 * cm, self.page_height - 3.85 * cm, 'Generated: ' + datetime.utcnow().replace(microsecond=0).strftime("%c"))  # noqa: E501

        else:
            canv.drawImage(os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../assets/images/Subastian_Profile.png')), 2.5 * cm, self.page_height - 4.5 * cm, width=80 * 1153/675, height=80, mask='auto')  # noqa: E501
            canv.setFont('Helvetica', 10)
            canv.drawString(7.5 * cm, self.page_height - 3.35 * cm, 'Generated: ' + datetime.utcnow().replace(microsecond=0).strftime("%c"))  # noqa: E501
        ...
        canv.drawImage(os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../assets/images/soi-logo.png')), self.page_width - 2 * cm - 40 * 161/122, 30, width=40 * 161/122, height=40, mask='auto')  # noqa: E501

        canv.restoreState()


class OneColumnTemplate(PageTemplate):  # pylint: disable=too-few-public-methods
    '''
...
class TOCTemplate(PageTemplate):  # pylint: disable=too-few-public-methods
    '''
...
class TwoColumnTemplate(PageTemplate):  # pylint: disable=too-few-public-methods
    '''
...
class RLDocTemplate(BaseDocTemplate):
    ...
    def afterInit(self):
        ...
        self.addPageTemplates(FrontCoverTemplate('Cover', title=self.title, subtitle=self.subtitle, pagesize=self.pagesize))  # noqa: E501
        ...
        self.seq = Sequencer()  # pylint: disable=attribute-defined-outside-init
