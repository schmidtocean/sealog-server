...
class LoweringVehicleReport(LoweringReportCreator):  # pylint: disable=too-few-public-methods
    ...
    def _build_vehicle_comp_datafile(self):
        ...
        path = os.path.join(CROPPED_DATA_DIR, self.cruise_record['cruise_id'], self.cruise_record['cruise_id'] + "_" + self.lowering_record['lowering_id'], "OpenRVDAS")  # noqa: E501
        # path = os.path.join(CRUISE_DATA_ROOT, self.cruise_record['cruise_id'], 'Vehicles', 'SuBastian', self.cruise_record['cruise_id'] + '_' + self.lowering_record['lowering_id'], 'OpenRVDAS')  # noqa: E501
        filename = self.cruise_record['cruise_id'] + '_sb_mech_comps_' + self.lowering_record['lowering_id'] + '.txt'  # noqa: E501
    ...
    def _import_comp_data(self, comp_file):
        ...
        # comp_data['Timestamp'] = pd.to_datetime(comp_data['date'] + ' ' + comp_data['time'], infer_datetime_format=True)  # noqa: E501
        comp_data['Timestamp'] = pd.to_datetime(comp_data['Timestamp'], utc=True)

        mask = (comp_data['Timestamp'] >= self.lowering_record['milestones']['start_dt']) & (comp_data['Timestamp'] <= self.lowering_record['milestones']['stop_dt'])  # noqa: E501
    ...
    def _build_comp_data_dive_summary(self, comp_data_headers, comp_data):
        ...
        comp_data_dive_summary_data = [
            ...
            [
                ...
                round(comp_data[-1][comp_data_headers.index('Comp1_%')] - comp_data[0][comp_data_headers.index('Comp1_%')], 4)  # noqa: E501
            ],
            [
                'Thrusters (Comp 2)',
                round(comp_data[0][comp_data_headers.index('Comp2_%')], 4),
                round(comp_data[-1][comp_data_headers.index('Comp2_%')], 4),
                round(comp_data[-1][comp_data_headers.index('Comp2_%')] - comp_data[0][comp_data_headers.index('Comp2_%')], 4)  # noqa: E501
            ],
            [
                'Term (Comp 3)',
                round(comp_data[0][comp_data_headers.index('Comp3_%')], 4),
                round(comp_data[-1][comp_data_headers.index('Comp3_%')], 4),
                round(comp_data[-1][comp_data_headers.index('Comp3_%')] - comp_data[0][comp_data_headers.index('Comp3_%')], 4)  # noqa: E501
            ],
            [
                'Submotor (Comp 4)',
                round(comp_data[0][comp_data_headers.index('Comp4_%')], 4),
                round(comp_data[-1][comp_data_headers.index('Comp4_%')], 4),
                round(comp_data[-1][comp_data_headers.index('Comp4_%')] - comp_data[0][comp_data_headers.index('Comp4_%')], 4)  # noqa: E501
            ],
            [
                'Elec Box (Comp 5)',
                round(comp_data[0][comp_data_headers.index('Comp5_%')], 4),
                round(comp_data[-1][comp_data_headers.index('Comp5_%')], 4),
                round(comp_data[-1][comp_data_headers.index('Comp5_%')] - comp_data[0][comp_data_headers.index('Comp5_%')], 4)  # noqa: E501
            ],
            [
                'Manip (Comp 6)',
                round(comp_data[0][comp_data_headers.index('Comp6_%')], 4),
                round(comp_data[-1][comp_data_headers.index('Comp6_%')], 4),
                round(comp_data[-1][comp_data_headers.index('Comp6_%')] - comp_data[0][comp_data_headers.index('Comp6_%')], 4)  # noqa: E501
            ]
        ]
    ...
    def _build_comp_data_on_bottom(self, comp_data_headers, comp_data):
        ...
        if not self.lowering_record['milestones']['on_bottom_dt'] or not self.lowering_record['milestones']['off_bottom_dt']:  # noqa: E501
            logging.warning("No On/Off Bottom milestones captured, can't build on bottom compensator stats.")  # noqa: E501
            return None

        mask = (comp_data[:, comp_data_headers.index('Timestamp')] >= self.lowering_record['milestones']['on_bottom_dt']) & (comp_data[:, comp_data_headers.index('Timestamp')] < self.lowering_record['milestones']['off_bottom_dt'])  # noqa: E501
    ...
    def _build_comp_data_plot(self, comp_data_headers, comp_data):  # pylint: disable=too-many-locals,too-many-statements  # noqa: E501
        ...
        start_ts = next((ts[0] for ts in comp_data[:, [comp_data_headers.index('Timestamp')]].astype(datetime) if ts is not None), None)  # noqa: E501
        ...
        x_ts = [datetime.utcfromtimestamp((tsStr[0] - start_ts).total_seconds()) for tsStr in comp_data[:, [comp_data_headers.index('Timestamp')]].astype(datetime)]  # noqa: E501

        comp_data_1 = [data[0] if data[0] else None for data in comp_data[:, [comp_data_headers.index('Comp1_%')]]]  # noqa: E501
        comp_data_2 = [data[0] if data[0] else None for data in comp_data[:, [comp_data_headers.index('Comp2_%')]]]  # noqa: E501
        comp_data_3 = [data[0] if data[0] else None for data in comp_data[:, [comp_data_headers.index('Comp3_%')]]]  # noqa: E501
        comp_data_4 = [data[0] if data[0] else None for data in comp_data[:, [comp_data_headers.index('Comp4_%')]]]  # noqa: E501
        comp_data_5 = [data[0] if data[0] else None for data in comp_data[:, [comp_data_headers.index('Comp5_%')]]]  # noqa: E501
        comp_data_6 = [data[0] if data[0] else None for data in comp_data[:, [comp_data_headers.index('Comp6_%')]]]  # noqa: E501

        comp_1, = ax_comp_1.plot(x_ts, comp_data_1, label='Main Res\n(Comp 1)', color='blue', linewidth=1)  # noqa: E501
        comp_2, = ax_comp_1.plot(x_ts, comp_data_2, label='Thrusters\n(Comp 2)', color='orange', linewidth=1)  # noqa: E501
        comp_3, = ax_comp_1.plot(x_ts, comp_data_3, label='Term\n(Comp 3)', color='green', linewidth=1)  # noqa: E501
        comp_4, = ax_comp_1.plot(x_ts, comp_data_4, label='Submotor\n(Comp 4)', color='red', linewidth=1)  # noqa: E501
        comp_5, = ax_comp_1.plot(x_ts, comp_data_5, label='Elec. Bo\n(Comp 5)', color='purple', linewidth=1)  # noqa: E501
        comp_6, = ax_comp_1.plot(x_ts, comp_data_6, label='Manip\n(Comp 6)', color='gray', linewidth=1)  # noqa: E501
        ...
        if 'vehicleRealtimeCTDData.depth_value' in self.lowering_data_headers:
            ...
            x_ts = [datetime.utcfromtimestamp((datetime.fromisoformat(tsStr[0]) - start_ts).total_seconds()) for tsStr in self.lowering_data[:, [self.lowering_data_headers.index('ts')]].astype(datetime)]  # noqa: E501
            ctd_depth_data = [float(depthStr[0]) if depthStr[0] else None for depthStr in self.lowering_data[:, [self.lowering_data_headers.index('vehicleRealtimeCTDData.depth_value')]]]  # noqa: E501
            ctd_temp_data = [float(tempStr[0]) if tempStr[0] else None for tempStr in self.lowering_data[:, [self.lowering_data_headers.index('vehicleRealtimeCTDData.temp_value')]]]  # noqa: E501

            ax_comp_2.plot(x_ts, ctd_depth_data, label='Depth', color='deepskyblue', linewidth=1, linestyle="dashed")  # noqa: E501
            ...
            ax_comp_3.plot(x_ts, ctd_temp_data, label='Temperature', color='limegreen', linewidth=1, linestyle="dashdot")  # noqa: E501
        ...
        ax_comp_1.legend(handles=[comp_1, comp_2, comp_3, comp_4, comp_5, comp_6], loc='lower left', bbox_to_anchor=(0.0, 1.01), ncol=6, borderaxespad=0, frameon=False, prop={"size": 8})  # noqa: E501
    ...
    def export_pdf(self):  # pylint: disable=too-many-locals,too-many-statements,too-many-branches
        ...
        if comp_datafile:
            ...
            comp_data_dive_summary_table = self._build_comp_data_dive_summary(comp_data_headers, comp_data)  # noqa: E501
            comp_data_on_bottom_table = self._build_comp_data_on_bottom(comp_data_headers, comp_data)  # noqa: E501
            ...
            if comp_data_plot_filename:
                ...
                comp_data_plot._restrictSize(PAGE_WIDTH - 2.5 * cm, 15 * cm)  # pylint: disable=protected-access  # noqa: E501
        ...
        if depths_plot_filename:
            ...
            depths_plot._restrictSize(PAGE_WIDTH - 5 * cm, PAGE_HEIGHT - 5 * cm)  # pylint: disable=protected-access  # noqa: E501
        ...
        if downcast_ctd_data_filename:
            ...
            downcast_ctd_data._restrictSize(PAGE_WIDTH - 5 * cm, 10 * cm)  # pylint: disable=protected-access  # noqa: E501
        ...
        if upcast_ctd_data_filename:
            ...
            upcast_ctd_data._restrictSize(PAGE_WIDTH - 5 * cm, 10 * cm)  # pylint: disable=protected-access  # noqa: E501
        ...
        if downcast_o2_data_filename:
            ...
            downcast_o2_data._restrictSize(PAGE_WIDTH - 5 * cm, 10 * cm)  # pylint: disable=protected-access  # noqa: E501
        ...
        if upcast_o2_data_filename:
            ...
            upcast_o2_data._restrictSize(PAGE_WIDTH - 5 * cm, 10 * cm)  # pylint: disable=protected-access  # noqa: E501
        ...
        if watch_change_filename:
            ...
            watch_change_plot._restrictSize(PAGE_WIDTH - 5 * cm, 10 * cm)  # pylint: disable=protected-access  # noqa: E501
        ...
        if depth_plot_filename:
            ...
            depth_plot._restrictSize(PAGE_WIDTH - 5 * cm, 10 * cm)  # pylint: disable=protected-access  # noqa: E501
        ...
        if dive_track_filename:
            ...
            dive_track._restrictSize(PAGE_WIDTH - 5 * cm, PAGE_HEIGHT - 5 * cm)  # pylint: disable=protected-access  # noqa: E501
        ...
        # flowables.append(Paragraph("<b>Cruise ID:</b> " + self.cruise_record['cruise_id'], self.body_text)),  # noqa: E501
        # flowables.append(Paragraph("<b>Cruise PI:</b> " + cruise_location, self.body_text)),
        # flowables.append(Paragraph("<b>Cruise Summary:</b> " + cruise_description, self.body_text)),  # noqa: E501
        ...
        if watch_change_table:
            ...
            flowables.append(Paragraph("No WATCH CHANGE events recorded for this dive", self.body_text))  # noqa: E501
            flowables.append(Spacer(PAGE_WIDTH, 1 * cm))
...
if __name__ == '__main__':
    ...
    try:
        ...
        OUTPUT_PATH = parsed_args.output_dir if parsed_args.output_dir else os.path.join(API_SERVER_FILE_PATH, 'lowerings', lowering_uid)  # noqa: E501
    ...
    # OUTPUT_FILENAME = cruise['cruise_id'] + '_Dive_' + args.lowering_id + '_Dive_Vehicle_Report.pdf'  # noqa: E501
