...
def combine_files_at_1hz(files, output_file):  # pylint: disable=too-many-locals
    """
    ...
    for f in files:
        try:
            ...
            headers = pl.read_csv(f, n_rows=1, truncate_ragged_lines=True).columns
            df = pl.read_csv(f, dtypes={col: pl.Utf8 for col in headers})
            df = df.with_columns(
    ...
    timestamps_1hz = pl.DataFrame({
        "Timestamp": pl.datetime_range(
            start=min_timestamp, end=max_timestamp, interval="1s", eager=True
        )
    })
