...
def auto_cast_mixed_numeric_columns(df: pl.DataFrame) -> pl.DataFrame:
    """
    Detect columns with mixed integer and float types and cast them to Float64.

    This helps avoid parsing issues where a column starts as integers but
    contains floats later in the file.

    Args:
        df (pl.DataFrame): Input DataFrame with potentially mixed types.

    Returns:
        pl.DataFrame: DataFrame with corrected column types.
    """
    for col in df.columns:
        ...
        if dtype in (
            ...
            try:
                casted = df[col].cast(pl.Float64)
                if not (casted.cast(dtype) == df[col]).all():
...
def combine_files_at_1hz(files, output_file):
    ...
    for f in files:
        df = pl.read_csv(
            f, infer_schema_length=10000, truncate_ragged_lines=True
        )
        ...
        df = auto_cast_mixed_numeric_columns(df)
    ...
    for df in dfs:
        ...
        df_filled = df_aligned.fill_null(strategy="forward")
        df_filled = df_filled.fill_null(strategy="backward")
