...
exports.plugin = {
  ...
  register: async (server, options) => {
    ...
    try {
      ...
      const init_data = filePreProcessor('./demo/FKt260806_cruiseRecord.json', 'cruises');
    ...
    try {
      ...
      const init_data = filePreProcessor('./demo/FKt260806_loweringRecords.json', 'lowerings');
      await collection.insertMany(init_data);
    }
    catch (err) {
      console.error('CREATE ERROR:', err.code);
    ...
    try {
      ...
      const init_data = filePreProcessor('./demo/FKt260806_E0018_eventOnlyExport.json', 'events');
      await collection.insertMany(init_data);
    }
    catch (err) {
      console.error('CREATE ERROR:', err.code);
    ...
    try {
      ...
      const init_data = filePreProcessor('./demo/FKt260806_E0018_auxDataExport.json', 'event_aux_data');
      await collection.insertMany(init_data);
    }
    catch (err) {
      console.error('CREATE ERROR:', err.code);
    ...
    try {
      ...
      const init_data = filePreProcessor('./demo/FKt260806_eventTemplates.json', 'event_templates');
      await collection.insertMany(init_data);
    }
    catch (err) {
      console.error('CREATE ERROR:', err.code);
